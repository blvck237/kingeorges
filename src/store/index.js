...
export default new Vuex.Store({
  ...
  actions: {
    ...
    increaseItem(context, product) {
      const cartItem = context.state.cart.find(item => item.id === product.id);
      if (cartItem) context.commit("incrementItemQuantity", cartItem);
    },
    decreaseItem(context, product) {
      const cartItem = context.state.cart.find(item => item.id === product.id);
      if (cartItem.quantity > 1)
        context.commit("decrementItemQuantity", cartItem);
      else context.commit("removeFromCart", cartItem);
    },
    setQuantity(context, product) {
      const cartItem = context.state.cart.find(item => item.id === product.id);
      context.commit("setItemQuantity", { cartItem, product });
    },
    checkOut(context, customer) {
      db.collection("proforma")
        .doc()
        .set({
          name: customer.name,
          type: customer.type,
